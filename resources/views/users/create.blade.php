...
<div class="col-md-8">
    ...
    <form enctype="multipart/form-data" class="bg-white shadow-sm p-3" action="{{ route('users.store') }}" method="POST">
        ...
        <br>
        <input type="checkbox" name="roles[]" id="ADMIN" value="ADMIN" {{ old('roles') && in_array('ADMIN', old('roles')) ? 'checked' : '' }}>
        <label for="ADMIN">Administrator</label>

        <input type="checkbox" name="roles[]" id="STAFF" value="STAFF" {{ old('roles') && in_array('STAFF', old('roles')) ? 'checked' : '' }}>
        <label for="STAFF">Staff</label>

        <input type="checkbox" name="roles[]" id="CUSTOMER" value="CUSTOMER" {{ old('roles') && in_array('CUSTOMER', old('roles')) ? 'checked' : '' }}>
